function [net,stats] = cnn_train_dag_xc(net, imdb, getBatch, varargin)
...
%     rng(epoch + opts.randomSeed) ;  
%     params.train = opts.train([1+randperm(numel(opts.train)-1) 1]) ;
%     params.train = [opts.train(2:end) opts.train(1)];    
    params.train = opts.train;
    params.val = opts.val;
  end
%   params.train = params.train(1:min(opts.epochSize, numel(opts.train)));
  params.train = params.train(1:min(opts.epochSize, numel(params.train)));
  params.imdb = imdb ;
